...
CLAY_DLL_EXPORT Clay_ElementId Clay__OpenElement(void);
...
Clay_ElementId Clay__OpenElement(void) {
    Clay_Context* context = Clay_GetCurrentContext();
    if (context->layoutElements.length == context->layoutElements.capacity - 1 || context->booleanWarnings.maxElementsExceeded) {
        ...
        return CLAY__INIT(Clay_ElementId) {0};
    }
    ...
    Clay_ElementId elementId = Clay__GenerateIdForAnonymousElement(openLayoutElement);
...
Clay_ElementId Clay__OpenTextElement(Clay_String text, Clay_TextElementConfig *textConfig) {
    ...
    if (context->layoutElements.length == context->layoutElements.capacity - 1 || context->booleanWarnings.maxElementsExceeded) {
        context->booleanWarnings.maxElementsExceeded = true;
        return CLAY__INIT(Clay_ElementId) { .id = 0, .offset = 0, .baseId = 0, .stringId = CLAY__STRING_DEFAULT };
    }
