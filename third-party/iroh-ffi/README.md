...
This repo defines Python, Swift, Kotlin, Node.js, and Zig bindings for [iroh](https://github.com/n0-computer/iroh), which is written in Rust. The generated bindings mirror the stabilized iroh 1.0 surface (endpoints, connections, paths, tickets, relays, services); the Zig binding currently provides the core identity, endpoint, connection, stream, datagram, and ticket APIs. Higher-level protocols not yet at 1.0 (`iroh-blobs`, `iroh-docs`, `iroh-gossip`) are out of scope.
...
- Zig 0.15.2 — source package in this repository
...
```sh
cargo make test-all          # rust + python + js + zig + kotlin + swift
