...
Two mutually-exclusive entry points (enforced by main()):
  ...
  1. VERSION positional arg — `cargo make prepare-release <V>` route. Bumps
     ...
     kotlin/{lib,android}/build.gradle.kts coordinates, build.zig.zon, and
     Package.swift releaseTag. Does NOT touch releaseChecksum.
...
def bump_zig(version: str) -> None:
    p = REPO / "build.zig.zon"
    s = p.read_text()
    new, n = re.subn(
        r'(?m)^    \.version = "[^"]+",$',
        f'    .version = "{version}",',
    ...
    if n != 1:
        sys.exit("could not find .version in build.zig.zon")
    p.write_text(new)
    print(f"  build.zig.zon version -> {version}")
