...
const NTSC_TICK_LENGTH_TABLE: [2][6]u64 = [_][8]i16{
...
pub const APU = struct {
    ...
    fn transfer(self: *Self) void {
        ...
        self.device.play(pulse_buffer.samples.items);
    }
        ...
        switch (addr) {
            ...
            0x4017 => {
                if (self.global_cyc % 2 == 0) {
                    ...
                    self.jitter = Jitter.Delay{ self.global_cyc + 1, value };
                }
            },
            else => {
                std.log.info("Attemtp to write to addr 0x{X:04}", .{addr});
            },
        }
    }
};
